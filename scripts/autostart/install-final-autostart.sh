...
# myMCPs Final Auto-start Installation Script
# This script installs user systemd service with proper auto-restart and monitoring

set -e

echo "🚀 Installing myMCPs Final Auto-start Service..."
...
print_status "INFO" "Installing final auto-start for user: $CURRENT_USER"
...
# Create the main service file with proper restart strategy
print_status "INFO" "Creating main service file with proper restart strategy..."
...
ExecStart=/bin/bash -c 'docker compose up -d && while true; do sleep 30; done'
...
print_status "PASS" "Main service file created with proper restart strategy"
...
print_status "INFO" "Creating startup script..."

cat > "$PROJECT_DIR/scripts/start-mcp-services.sh" << 'EOF'
#!/bin/bash
...
chmod +x "$PROJECT_DIR/scripts/start-mcp-services.sh"
print_status "PASS" "Startup script created"

# Update the main service to use the startup script
print_status "INFO" "Updating main service to use startup script..."
...
Type=simple
WorkingDirectory=$PROJECT_DIR
ExecStart=$PROJECT_DIR/scripts/start-mcp-services.sh
ExecStop=/bin/bash -c 'cd $PROJECT_DIR && docker compose down'
TimeoutStartSec=300
TimeoutStopSec=60
Environment=HOME=$HOME
Environment=USER=$CURRENT_USER
...
print_status "PASS" "Main service updated to use startup script"
...
echo "   - Continue running independently"
echo ""
